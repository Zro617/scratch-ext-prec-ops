...
(function (ext) {
    ...
    ext.do_add = function (n1, n2) {
        ...
        // add a decimal place if only one summand has it
        if      (n1_decCount > 0 && n2_decCount == 0) n1 += '.';
		else if (n1_decCount == 0 && n2_decCount > 0) n2 += '.';

		// remove negation signs
		if (n1_signed) n1 = negate(n1);
		if (n2_signed) n2 = negate(n2);
        ...
        for (var idx = 0; idx < result.length - 1; idx++) {
            n1_digit = n1[n1.length - (idx + 1)];
            n2_digit = n2[n2.length - (idx + 1)];
            if (n1_signed === n2_signed) {
               calc = n1_digit + n2_digit + carry;
			} else {
                calc = ((n1_signed) ? (n2_digit - n1_digit) : (n1_digit - n2_digit)) + carry;
            }
		...
		if (n1_decCount > 0) result.insert(result.length - n1_decCount, '.');
	...
	ext.do_mult = function (n1, n2) {
		// Preventive measure for numbers within native accuracy
		if (n1.length + n2.length < 13) return n1 * n2;
	...
	ext.check_sign = function (n, sign) {
		...
		switch (sign) {
			...
			case 'negative': return (n.indexOf('-') == 0);
			case 'positive': return (n.indexOf('-') == -1);
		}
	};
	
	ext.check_num = function (n, type) {
		switch (type) {
			case 'integer': return (!n.isNaN && n.indexOf('.') == -1);
			case 'decimal': return (!n.isNaN && n.indexOf('.') > -1);
			case 'NaN': return n.isNaN;
			case 'infinity': return !n.isFinite;
		}
	};
	
	
	// Insert is not native to JavaScript
	Array.prototype.insert = function (idx, x) { this.splice(idx, 0, x); };
